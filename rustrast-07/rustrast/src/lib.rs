...
struct SceneBuffers {
    ...
    intensities: RefCell<Vec<__m256>>,
...
pub fn init() {
    ...
    let scene = SceneBuffers {
        ...
        intensities: RefCell::new(iter::repeat(unsafe { _mm256_setzero_ps() }).take(num_vertices_padded).collect()),
...
fn draw_tile<T : Send + Copy>(
        ...
        extras: &[T], shader: &impl AvxFragmentShader<T>) {
    ...
    for i in 0..NUM_BIN_THREADS {
        for j in 0..triangles[i].len() {
            ...
            let e0 = extras[model.trianglev0s[it] as usize];
            let e1 = extras[model.trianglev1s[it] as usize];
            let e2 = extras[model.trianglev2s[it] as usize];
            fill_triangle(&mut tile.colour, &mut tile.depth, it, xmin, ymin, xmax, ymax, x0, y0, z0, iw0, x1, y1, z1, iw1, x2, y2, z2, iw2, e0, e1, e2, iarea, tl, shader);
        }
    }
}
...
impl <'a>GouraudVertexShader<'a> {
    fn new(model: &'a Model, t: &'a Transformation, light_direction: &'a CartesianVector, light_intensity: f32, ambient_intensity: f32, it_world: &'a[[f32; 3]; 3]) -> Self {
        ...
        Self { model, t, light_direction_x, light_direction_y, light_direction_z, light_intensity, ambient_intensity, it_world }
    }
}

impl AvxVertexShader<__m256> for GouraudVertexShader<'_> {
    unsafe fn vertex(&self, iv_offset: usize,
            xs_out: &mut [__m256], ys_out: &mut [__m256], zs_out: &mut [__m256], iws_out: &mut [__m256], extras_out: &mut [__m256],
        ...
        vectors_chunk_transformed_normalised!(vertex_normal_xs, vertex_normal_ys, vertex_normal_zs, self.it_world, |i, xt, yt, zt| {
            let diffuse = _mm256_mul_ps(_mm256_max_ps(dot_product!(xt, yt, zt, self.light_direction_x, self.light_direction_y, self.light_direction_z), _mm256_setzero_ps()), self.light_intensity);
            let intensity = _mm256_min_ps(_mm256_add_ps(diffuse, self.ambient_intensity), _mm256_set1_ps(1.0));
            let mut intensity_arr: [f32; 8] = [0.0; 8];
            _mm256_storeu_ps(intensity_arr.as_mut_ptr(), intensity);
            for j in 0..8 {
                extras_out[i * 8 + j] = _mm256_set1_ps(intensity_arr[j]);
            }
        });
    }
}
...
impl GouraudFragmentShader {
    fn new() -> Self {
        Self{}
    }
}

impl AvxFragmentShader<__m256> for GouraudFragmentShader {
    unsafe fn fragment(&self, _it: usize, _w0: __m256, _w1: __m256, _w2: __m256, p_w0: __m256, p_w1: __m256, p_w2: __m256, intensity0: __m256, intensity1: __m256, intensity2: __m256) -> __m256i {
...
pub fn draw(buffer: *mut RGBQUAD, width: usize, height: usize, stride: usize) {
    ...
    let vertex_shader = GouraudVertexShader::new(&model, &t, &light_direction, 0.3, 0.05, &it_world);
    {
        ...
        time(format!("Transformed and shaded {} vertices", num_vertices), || {
            //execute_vertex_shader(xs_out, ys_out, zs_out, iws_out, intensities_out, model, &vertex_shader)
            execute_vertex_shader(xs_out, ys_out, zs_out, iws_out, intensities_out, model, &vertex_shader)
        });
    }
...

    {
        ...
        time(format!("Calculated {} sets of properties", num_triangles), || {
        ...
        pool.scoped(|scope| {
            ...
            while ymin < height  {
                ...
                while xmin < stride {
                    ...
                    scope.execute(move || {
                        let fragment_shader = GouraudFragmentShader::new();
                        draw_tile(&mut tile, model, xs, ys, zs, iws, bounds, iareas, tls, triangles, intensities, &fragment_shader);
                    });
