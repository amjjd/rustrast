...
... or, seven operations per average calculation (of which one is needed per eight output pixels for 2x MSAA and three
for 4x MSAA):
...
The result then has to be gamma corrected and converted to RGB8 which can be done by or-ing together a lookup per
...
Unfortunately this is only very slightly better than averaging by extracting components (ten operations for 2x MSAA,
twenty for 4x MSAA), averaging using `_mm256_avg_epu16` (three and nine respectively), and gamma correcting (three
gathers plus two more operations in both modes).

Quality is remembered long after price is forgotten
---------------------------------------------------

After a lot of frustration caused by missing the fact that storing fragment colours out of order meant I needed to
vertically align fragment blocks, I found that 2x MSAA wasn't quite right:

![2x MSAA](./2xMSAA.png)

Note how poorly it handles edges where the slope is similar to how the subsamples are distributed in the pixel:

![2x MSAA not working well](./2xMSAA_still_jagged.png)

Searching suggested that an old technique called temporal antialiasing improved this by using a complementary subsample
pattern each alternate frame. I'm also sure I saw something online that suggested using a different pattern on alternate
pixels, so I tried that. It's absolutely free, and makes a meaningful improvement to the image quality:
...
1.5ms per frame. However the cost is from processing full colour rather than grayscale so is necessary. Over and above
that extra 1.5ms, supersampling adds about 12ms per frame; 4x MSAA about 8ms; and 2x MSAA about 3ms. 
...
I don't think I really learned any Rust this time. I certainly used const generics a lot.

Next, it's finally time to say goodbye to the Peter Dinklage model, because it doesn't have any [textures](../rustrast-10/).
