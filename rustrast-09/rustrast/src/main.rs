...
lazy_static! {
    static ref BACK_BUFFER: Mutex<BackBuffer> = Mutex::new(BackBuffer{
        ...
        dc: CreatedHDC(0),
        old_object: HGDIOBJ(0),
        size: BufferSize { stride: 0, lines: 0 },
        bitmap: HBITMAP(0),
        buffer: null_mut()
    });
}
...
unsafe extern "system" fn window_proc(hwnd: HWND, msg: u32, w_param: WPARAM, l_param: LPARAM) -> LRESULT {
    match msg {
        ...
        WM_SIZE => {
            ...
            if back_buffer.client_area_width == 0 || back_buffer.client_area_height == 0 {
                return LRESULT(0);
            }

            if back_buffer.bitmap.0 != 0 {
                SelectObject(back_buffer.dc, back_buffer.old_object);
            ...
            back_buffer.size = get_out_buffer_size(back_buffer.client_area_width, back_buffer.client_area_height);

            let bitmap_info = BITMAPINFO {
                bmiHeader: BITMAPINFOHEADER {
                    biSize: size_of::<BITMAPINFOHEADER>() as u32,
                    biWidth: back_buffer.size.stride as i32,
                    biHeight: -(back_buffer.size.lines as i32),
                    biPlanes: 1,
                    biBitCount: 32,
            ...
            back_buffer.old_object = SelectObject(back_buffer.dc, back_buffer.bitmap);
        ...
        WM_PAINT => {
            ...
            let buffer_slice = std::slice::from_raw_parts_mut(back_buffer.buffer, back_buffer.size.len());
            time("Drew to back buffer", || {
                draw(buffer_slice, &back_buffer.size, back_buffer.client_area_width, back_buffer.client_area_height);
            });

            // copy to screen
            time("BitBlted to screen", || {
                BitBlt(
                    hdc,
                    0, 0,
